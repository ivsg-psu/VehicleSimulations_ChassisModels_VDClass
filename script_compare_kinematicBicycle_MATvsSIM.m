...
steeringSIM = steering_amplitude_degrees*pi/180*sin((2*pi/Period)*(simulationTimes)); % steering angle
steeringMAT = steering_amplitude_degrees*pi/180*sin((2*pi/Period)*(simulationTimes)); % steering angle
...
mdl = 'mdl_VD_KinematicBicycleModel.slx';           % model name (open or on path)
...
for ith_time = 1:N_timeSteps
	...
	[~, y] = fcn_VD_RungeKutta(...
		@(t,y) fcn_VD_kinematicBicycleModel(y,inputOmega, U, L), ...
...
subplot(3,1,3);
plot(simulationTimes, abs(all_Ymat - (all_Y+ 0*eps)));
xlabel('Time [s]','Interpreter','Latex','Fontsize',18)

...
%   ref_normal_force, normal_force: A Nx2 matrix of normal force(s) [N]
...
subplot(2,1,1)
plot(ref_time, ref_normal_force(:,1), 'b', 'Linewidth', 1.2)
hold on
plot(time, normal_force(:,1), 'r--', 'Linewidth', 1.2)
grid on
legend(['Front ' ref_sim_name], ['Front ' sim_name], 'Location', 'best',...
       'Interpreter','Latex','Fontsize',13)
set(gca,'Fontsize',13)
ylabel('Normal Force $[N]$','Interpreter','Latex','Fontsize',13)
ylim([min_value-offset max_value+offset])

subplot(2,1,2)
plot(ref_time, ref_normal_force(:,2), 'b', 'Linewidth', 1.2)
hold on
plot(time, normal_force(:,2), 'r--', 'Linewidth', 1.2)
grid on

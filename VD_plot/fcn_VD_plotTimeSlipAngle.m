function fcn_VD_plotTimeSlipAngle(time, slipAngles, varargin)
%% fcn_VD_plotTimeSlipAngle
%   fcn_VD_plotTimeSlipAngle plots the slip angle(s) versus time where the
%   columns are slip angles of each tire. 
%
% FORMAT:
%
%       fcn_VD_plotTimeSlipAngle(time, slipAngles, (figNum))
%
% INPUTS:
%
%     time: an Nx1 vector representing time [sec]
%
%     slipAngles: an NxM vector representing the slip angle(s) of M tires
%     [rad], in the following order: FL, FR, RL, RR if 4 columns, or F and
%     R if 2 columns
%
%     (OPTIONAL INPUTS)
%
...
% EXAMPLES:
%
%     See the script: script_test_fcn_VD_plotTimeSlipAngle
...
% - Renamed "slip_+angle" variable to be slipAngles
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the slipAngles input to be sure it has 1 column and 1 row, positive
        fcn_DebugTools_checkInputsToFunctions(slipAngles, '1orMorecolumn_of_numbers',[Ndata Ndata]);
...
if flag_do_plots
    ...
    max_value = max(slipAngles, [], 'all');
    min_value = min(slipAngles, [], 'all');
    ...
    for ith_tire = 1:Ntires
        plot(time,slipAngles(:,ith_tire),colorStrings{ith_tire},'Linewidth',lineWidths(ith_tire), 'DisplayName', labelStrings{ith_tire});
    end

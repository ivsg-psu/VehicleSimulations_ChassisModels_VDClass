function fcn_VD_plotTimeWheelTorque(time, wheelTorques, varargin)
%% fcn_VD_plotTimeWheelTorque
%   fcn_VD_plotTimeWheelTorque plots the wheel torque(s) versus
%   time where the columns for each tire.
%
% FORMAT:
%
%       fcn_VD_plotTimeWheelTorque(time, wheelTorques, (figNum))
%
% INPUTS:
%
%     time: an Nx1 vector representing time [sec]
%
%     wheelTorques: an NxM vector representing the longitudinal wheel torque(s)
%     of M tires [N-m], in the following order: FL, FR, RL, RR if 4
...
% EXAMPLES:
%
%     See the script: script_test_fcn_VD_plotTimeWheelTorque
...
% - Renamed "slip_+angle" variable to be wheelTorques
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the wheelTorques input to be sure it has 1 column and 1 row, positive
        fcn_DebugTools_checkInputsToFunctions(wheelTorques, '1orMorecolumn_of_numbers',[Ndata Ndata]);
...
if flag_do_plots
    ...
    max_value = max(wheelTorques, [], 'all');
    min_value = min(wheelTorques, [], 'all');
    ...
    for ith_tire = 1:Ntires
        plot(time,wheelTorques(:,ith_tire),colorStrings{ith_tire},'Linewidth',lineWidths(ith_tire), 'DisplayName', labelStrings{ith_tire});
    end
    ...
    set(gca,'Fontsize',13)
    ylabel('Wheel Torque $[N-m]$','Interpreter','latex','Fontsize',18)

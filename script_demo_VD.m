...
%
%% FIX --> https://github.com/ivsg-psu/PathPlanning_MapTools_ParseXODR
%
...
% * GeometryClass - contains core geometric calculations
...
if 1==0
    clear flag_VD_Folders_Initialized;
    fcn_INTERNAL_clearUtilitiesFromPathAndFolders;
end

%% Do we need to set up the work space?
if ~exist('flag_VD_Folders_Initialized','var')
    this_project_folders = {'VD_Utilities','VD_Utilities_Bicycle','VD_Utilities_DualTrack','DualTrack_Simulink','DualTrack_MATLAB','Bicycle_Simulink','Bicycle_Python','Bicycle_MATLAB','Datafiles'};
    fcn_INTERNAL_initializeUtilities(library_name,library_folders,library_url,this_project_folders);
    flag_VD_Folders_Initialized = 1;
end
...
if ~exist(flag_varname,'var') || isempty(eval(flag_varname))
    ...
    if flag_allFoldersThere==0
        ...
        flag_delete_it = 0;
        if exist(dependency_url,'file')
            zip_file_name = dependency_url;
        else
            try
                zip_file_name = websave(save_file_name,dependency_url);
            catch ME
                warning('The URL link did not work. Is the library public?');
                rethrow(ME)
            end
            flag_delete_it = 1;
        end

...
for counter in range(N_timeSteps):
	...
	## Steering Input
	delta_f = (1-1*(0<input_time-Period))*steering_amplitude*np.sin((2*np.pi/Period)*input_time) # front steering angle [rad]
	steering_angle = np.array([delta_f, 0])

	globals.flag_update = True # set it to to true before every call to RK4 method
	y = fcn_VD_RungeKutta(lambda t_start,y_in: bicycle_model.fcn_VD_st5dofModel(t_start,y_in,steering_angle,wheel_torque,\
																	            vehicle,road_properties,friction_coefficient,type_of_transfer),\
						  					   input_states,input_time,deltaT)

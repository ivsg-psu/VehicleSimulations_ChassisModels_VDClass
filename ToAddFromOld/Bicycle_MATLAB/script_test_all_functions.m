...
repoShortName = '_VD_';
functionsFolderString = 'Bicycle_MATLAB';
% functionsFolderString = 'Functions';
...
rootFilePath = extractBefore(filepath,functionsFolderString);
...
if sumOfmFiles~=1
    fcn_DebugTools_cprintf('*Red','FAILED... %.0f were found in folder: \n\t\t%s!\n', sumOfmFiles, rootFilePath);
else
...
fucntionsFolder = fullfile(pwd,functionsFolderString);

fcn_DebugTools_cprintf('*blue','Checking functions folder:\n\t%s:\n', fucntionsFolder);

functionsDirectoryQueryAllFiles = fullfile(pwd,functionsFolderString,'*.*');
% Use the following instead, if wish to do subdirectories
% directoryQuery = fullfile(pwd,functionsFolderString,'**','*.*');
...
functionsDirectoryQueryFunctionsOnly = fullfile(pwd,functionsFolderString,'fcn_*.*');
% Use the following instead, if wish to do subdirectories
% directoryQuery = fullfile(pwd,functionsFolderString,'**','*.*');
...
if 1==0
    functionsFolder = fullfile(pwd,functionsFolderString);
        ...
        if 1==0
            ...
            if 1==0
                if sum(flagsStringWasFoundInFilesRaw)>0
                    ...
                    functionsDirectory = fullfile(pwd,functionsFolderString);

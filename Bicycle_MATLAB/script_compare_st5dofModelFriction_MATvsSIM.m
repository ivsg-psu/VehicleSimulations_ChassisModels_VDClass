%% script_compare_st5dofModelFriction_MATvsSIM.m
...
sim('mdl_VD_st5dofModelFriction.slx', TotalTime);
...
matlab_friction = nan(N_timeSteps,2);
...
if 1==counter
    ...
    estimated_accel = matlab_States(counter-1,6:7)'+...
        [normrnd(0,0.5)*(vdParam.sampling_time_imu^2); ...
        normrnd(0,0.5)*(vdParam.sampling_time_imu^2)];
end
slip_angle = fcn_VD_stSlipAngle(estimated_vel(1),estimated_vel(2),...
    estimated_vel(3),steering_angle,vehicle);
normal_force = fcn_VD_stNormalForce(estimated_accel,vehicle,road_properties,...
    type_of_transfer);
friction_estimate = fcn_VD_stEstimateFrictionCoefficient(slip_angle,...

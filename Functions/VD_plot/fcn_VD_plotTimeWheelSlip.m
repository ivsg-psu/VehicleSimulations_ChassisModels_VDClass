function fcn_VD_plotTimeWheelSlip(time, wheelSlips, varargin)
%% fcn_VD_plotTimeWheelSlip
%   fcn_VD_plotTimeWheelSlip plots the longitudinal wheel slip(s) versus
%   time where the columns are wheel slips of each tire.
%
% FORMAT:
%
%       fcn_VD_plotTimeWheelSlip(time, wheelSlips, (figNum))
%
% INPUTS:
%
%     time: an Nx1 vector representing time [sec]
%
%     wheelSlips: an NxM vector representing the longitudinal wheel slip(s)
%     of M tires [unitless], in the following order: FL, FR, RL, RR if 4
...
% EXAMPLES:
%
%     See the script: script_test_fcn_VD_plotTimeWheelSlip
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the wheelSlips input to be sure it has 1 column and 1 row, positive
        fcn_DebugTools_checkInputsToFunctions(wheelSlips, '1orMorecolumn_of_numbers',[Ndata Ndata]);

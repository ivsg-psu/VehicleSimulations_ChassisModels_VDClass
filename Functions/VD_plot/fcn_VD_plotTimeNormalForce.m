function fcn_VD_plotTimeNormalForce(time, normalForces, varargin)
%% fcn_VD_plotTimeNormalForce
%   fcn_VD_plotTimeNormalForce plots the longitudinal wheel slip(s) versus
%   time where the columns are wheel slips of each tire.
%
% FORMAT:
%
%       fcn_VD_plotTimeNormalForce(time, normalForces, (figNum))
%
% INPUTS:
%
%     time: an Nx1 vector representing time [sec]
%
%     normalForces: an NxM vector representing the normal force(s)
%     of M tires [N], in the following order: FL, FR, RL, RR if 4
...
% EXAMPLES:
%
%     See the script: script_test_fcn_VD_plotTimeNormalForce
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check the normalForces input to be sure it has 1 column and 1 row, positive
        fcn_DebugTools_checkInputsToFunctions(normalForces, '1orMorecolumn_of_numbers',[Ndata Ndata]);
...
if flag_do_plots
    ...
    max_value = max(normalForces, [], 'all');
    min_value = min(normalForces, [], 'all');
    ...
    for ith_tire = 1:Ntires
        plot(time,normalForces(:,ith_tire),colorStrings{ith_tire},'Linewidth',lineWidths(ith_tire), 'DisplayName', labelStrings{ith_tire});
    end
    ...
    set(gca,'Fontsize',13)
    ylabel('Normal Force $[N]$','Interpreter','latex','Fontsize',18)

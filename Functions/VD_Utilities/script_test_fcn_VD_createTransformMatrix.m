...
rotations = [0 0 15]*pi/180; % radians
translations = [1 2 3]; % radians
...
assert(isnumeric(transformationMatrix));

% Check variable sizes
assert(size(transformationMatrix,1)==4); 
...
% Set the input arguments
rotations = [1; 1; pi/4]; % meters, meters, radians
inputOmega = -100*pi/180; % radians
U = 20; % meters per second

% Call the function
dydt = fcn_VD_createTransformMatrix( rotations, inputOmega, U, ([]));

% Check variable types
assert(isnumeric(dydt));

% Check variable sizes
assert(size(dydt,1)==3); 
assert(size(dydt,2)==1); 

% Check variable values
assert(isequal(dydt, [U*cos(rotations(3)); U*sin(rotations(3)); inputOmega])); 
...
% Call the function
dydt = fcn_VD_createTransformMatrix( rotations, inputOmega, U, (-1));

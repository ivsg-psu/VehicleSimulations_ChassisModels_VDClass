% script_test_fcn_VD_body2GlobalCoordinateRK4.m
% tests fcn_VD_body2GlobalCoordinateRK4.m
...
yawrate = 2*pi/180;
inputsVsTime = [simulationTimes yawrate*ones(length(simulationTimes),1)];
predictedRadius = parameters.U/yawrate;
...
% Pull out the diameter (approximately) by finding the max global Y value
estimatedDiameter = max(stateTrajectory(:,2));
estimatedRadius = estimatedDiameter/2;
assert(isequal(round(estimatedRadius,2),round(predictedRadius,2))) % Do they agree to 2 decimal places?

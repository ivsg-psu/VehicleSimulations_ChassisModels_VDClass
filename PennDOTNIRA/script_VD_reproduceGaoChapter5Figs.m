%% script_reproduceGaoChapter5Figs
...
R_turn = 190;   % turn radius, m (Gao Fig 5-5 top)
k_turn = 1 / R_turn; % arc curvature 5e-3
L_spir = 150; % clothoid length, m
dHead_spir = L_spir * (0 + k_turn) / 2; % heading change per clothoid
arcAng_180 = pi - 2 * dHead_spir;  % arc angle for a full pi turn

L_bot_pre = 150;  % from start heading east
L_top = 300; % top straight (west)
L_bot_post = 150; % closing the loop back to start
...
roadSpec(1).length = L_bot_pre;
roadSpec(1).grade  = [];

roadSpec(2).type    = 'spiral';    
roadSpec(2).length  = L_spir;
roadSpec(2).k_start = 0;           
roadSpec(2).k_end   = k_turn;
roadSpec(2).grade   = [];

roadSpec(3).type    = 'arc';        
roadSpec(3).radius  = R_turn;
roadSpec(3).angle   = arcAng_180;
roadSpec(3).grade   = [];

roadSpec(4).type    = 'spiral';    
roadSpec(4).length  = L_spir;
roadSpec(4).k_start = k_turn;      
roadSpec(4).k_end   = 0;
roadSpec(4).grade   = [];

roadSpec(5).type   = 'straight';   
roadSpec(5).length = L_top;
roadSpec(5).grade  = [];

roadSpec(6).type    = 'spiral';    
roadSpec(6).length  = L_spir;
roadSpec(6).k_start = 0;           
roadSpec(6).k_end   = k_turn;
roadSpec(6).grade   = [];

roadSpec(7).type   = 'arc';        
roadSpec(7).radius = R_turn;
roadSpec(7).angle  = arcAng_180;
roadSpec(7).grade  = [];

roadSpec(8).type    = 'spiral';    
roadSpec(8).length  = L_spir;
roadSpec(8).k_start = k_turn;      
roadSpec(8).k_end   = 0;
roadSpec(8).grade   = [];

roadSpec(9).type   = 'straight';   
roadSpec(9).length = L_bot_post;
roadSpec(9).grade  = [];
...
%% Build the road (with zero grade), then override theta to match Gao
...
a_total = sqrt(ax_profile.^2 + ay_profile.^2);
a_max  = mu * speedPlannerSpecs.g .* cos(road.theta) + speedPlannerSpecs.g * abs(sin(road.theta));
...
plot(road.s, ax_profile, '-.', 'LineWidth', 1.5, 'Color', [0.2 0.2 0.9], ...
    'DisplayName', 'a_x');

plot(road.s, ay_profile, '--', 'LineWidth', 1.5, 'Color', [0.2 0.7 0.2], ...
    'DisplayName', 'a_y');

plot(road.s, a_total, ':', 'LineWidth', 1.8, 'Color', [0.9 0.1 0.1], ...
    'DisplayName', 'a_{total}');
...
%% Feasibility check: the actual friction ellipse constraint
...
plot(road.s, a_total_eff, 'r-', 'LineWidth', 1.5, ...
     'DisplayName', '\surd((a_x + g sin\theta)^2 + a_y^2)');

plot(road.s, a_bound_ellipse, 'k--', 'LineWidth', 1.5, ...
     'DisplayName', '\lambda\mug cos\theta');
...
assert(all(a_total_eff <= a_bound_ellipse + 5e-2), 'Speed planner produced an infeasible profile (violates friction ellipse).');
...
mask_dryArc = (road.kappa >= 0.9 * k_turn) & (mu >= 0.8);
if any(mask_dryArc)
...
mask_ice = (mu < 0.2);
if any(mask_ice)
    fprintf(1, 'Curve limit on icy patch (mu=0.15): %.2f m/s\n', min(speedData.U_corner(mask_ice)));
end
